...
class FuturePredictionDataset(torch.utils.data.Dataset):
    ...
    def prepro(self):
        ...
        samples.sort(key=lambda x: (self.nusc.get("scene", x['scene_token'])['name'], x['timestamp']))
    ...
    def get_indices(self):
        ...
        for index in range(len(self.ixes)):
            ...
            if scene_token_mapping[self.ixes[index]['scene_token']] not in selected_indices:
                continue
            is_valid_data = True
            previous_rec = None
            current_indices = []
    ...
    def __getitem__(self, index):
        ...
        for i, index_t in enumerate(self.indices[index]):
            ...
            if i < self.receptive_field:
                ...
                data['point_clouds'].append(point_clouds)
        ...
        # images, intrinsics, extrinsics, depths, unnormalized_images, rotations, translations, camera_intrinsics, point_clouds

        # for key, value in data.items():
        #     if key in ['image', 'segmentation', 'instance', 'future_egomotion', 'hdmap', 'pedestrian', 'unnormalized_images']:
        #         if key == 'depths' and self.cfg.LIFT.GT_DEPTH is False:
        #             continue
        #         data[key] = torch.cat(value, dim=0)
