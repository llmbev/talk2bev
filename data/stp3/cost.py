...
class Cost_Function(nn.Module):
    ...
    def forward(self, cost_volume, trajs, semantic_pred, lane_divider, drivable_area, target_point):
        ...
        cost_fo = safetycost + headwaycost + lrdividercost + rulecost + costvolume
...
class BaseCost(nn.Module):
    ...
    def evaluate(self, trajs, C):
        ...
        returns:
            CS: torch.Tensor<float> (B, N, n_future)
        '''
        B, N, n_future, _ = trajs.shape
...
class Comfort(BaseCost):
    ...
    def forward(self, trajs, t_fin=3):
        '''
        trajs: torch.Tensor<float> (B, N, n_future, 2)
        '''
        B, N, n_future, _ = trajs.shape
        #  3s in n_future time steps
        frame_rate = t_fin / (n_future)
        ...
        for i in range(n_future):
            if i == 0:
                lateral_velocity[:,:,i] = trajs[:,:,i,0] / frame_rate
                longitudinal_velocity[:,:,i] = trajs[:,:,i,1] / frame_rate
            else:
                lateral_velocity[:,:,i] = (trajs[:,:,i,0] - trajs[:,:,i-1,0]) / frame_rate
                longitudinal_velocity[:,:,i] = (trajs[:,:,i,1] - trajs[:,:,i-1,1]) / frame_rate
        for i in range(1, n_future):
            lateral_acc[:,:,i] = (lateral_velocity[:,:,i] - lateral_velocity[:,:,i-1]) / frame_rate
            longitudinal_acc[:,:,i] = (longitudinal_velocity[:,:,i] - longitudinal_velocity[:,:,i-1]) / frame_rate
        ...
        for i in range(n_future):
            if i == 0:
                ego_velocity[:, :, i] = torch.sqrt((trajs[:, :, i] ** 2).sum(dim=-1)) / frame_rate
            else:
                ego_velocity[:, :, i] = torch.sqrt(((trajs[:, :, i] - trajs[:, :, i - 1]) ** 2).sum(dim=-1)) / frame_rate
        for i in range(1, n_future):
            ego_acc[:,:,i] = (ego_velocity[:,:,i] - ego_velocity[:,:,i-1]) / frame_rate
        for i in range(2, n_future):
            ego_jerk[:,:,i] = (ego_acc[:,:,i] - ego_acc[:,:,i-1]) / frame_rate
        ego_jerk,_ = torch.abs(ego_jerk).max(dim=-1)

        subcost = torch.zeros((B, N), device=trajs.device)

        # 30 was the clamp for 0.5 frame_rate 
        lateral_clamp = 30
        lateral_acc = torch.clamp(torch.abs(lateral_acc) - self.c_lat_acc, 0,lateral_clamp)
        subcost += lateral_acc ** 2

        # 30 was the clamp for 0.5 frame_rate
        longitudinal_clamp = 30
        longitudinal_acc = torch.clamp(torch.abs(longitudinal_acc) - self.c_lon_acc, 0, longitudinal_clamp)
        subcost += longitudinal_acc ** 2

        # 20 was the clamp for 0.5 frame rate
        jerk_clamp = 20
        ego_jerk = torch.clamp(torch.abs(ego_jerk) - self.c_jerk, 0, jerk_clamp)
        subcost += ego_jerk ** 2
...
class ComfortMetric():
    def __init__(self):
        super(ComfortMetric, self).__init__()
        """
        This was the value for 0.5 frame rate
            self.c_lat_acc = 3 # m/s2
            self.c_lon_acc = 3 # m/s2
            self.c_jerk = 1 # m/s3
        """
    ...
    def compute_cost(self, trajs, t_fin=3):
        '''
        trajs: torch.Tensor<float> (B, N, n_future, 2)
        '''
        #  3s in n_future time steps
