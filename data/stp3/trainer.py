    ...
    def shared_step(self, batch, is_train):
        ...
        if is_train:
            ...
            if self.cfg.PLANNING.ENABLED:
                ...
                pl_loss, final_traj, CS, cur_trajs, gt_cost_fc, gt_cost_fo, final_traj, sm_cost_fc, sm_cost_fo = self.model.planning(
        ...
        # Metrics
            ...
            if self.cfg.PLANNING.ENABLED:
                ...
                _, final_traj, CS, cur_trajs, gt_cost_fc, gt_cost_fo, final_traj, sm_cost_fc, sm_cost_fo = self.model.planning(
                    cam_front=output['cam_front'].detach(),
                    trajs=trajs[:, :, 1:],
                    gt_trajs=labels['gt_trajectory'][:, 1:],
    ...
    def prepare_future_labels(self, batch):
        ...
        if self.cfg.SEMANTIC_SEG.HDMAP.ENABLED:
            ...
            hdmap_labels_past = cumulative_warp_features(
                hdmap_labels[:, :self.model.receptive_field].float(),
                future_egomotion[:, :self.model.receptive_field],
                mode='nearest', spatial_extent=self.spatial_extent,
            ).long().contiguous()[:, :-1]
            hdmap_labels = cumulative_warp_features_reverse(
                hdmap_labels[:, (self.model.receptive_field - 1):].float(),
                future_egomotion[:, (self.model.receptive_field - 1):],
                mode='nearest', spatial_extent=self.spatial_extent,
            ).long().contiguous()
