...
class Planning(nn.Module):
    ...
    def loss(self, trajs, gt_trajs, cost_volume, semantic_pred, lane_divider, drivable_area, target_points):
        ...
        '''
        sm_cost_fc, sm_cost_fo = self.cost_function(cost_volume, trajs[:, :, :, :2], semantic_pred, lane_divider, drivable_area, target_points)
        print(sm_cost_fc.shape, sm_cost_fo.shape, "sm_costs")
        ...
        return torch.mean(L), gt_cost_fc, gt_cost_fo, sm_cost_fc, sm_cost_fo
    ...
    def forward(self,cam_front, trajs, gt_trajs, cost_volume, semantic_pred, hd_map, commands, target_points):
        ...
        print(cur_trajs.shape, "cur_trajs", trajs.shape, "trajs")
        cur_trajs = trajs
        ...
        if self.training:
            loss, gt_cost_fc, gt_cost_fo, sm_cost_fc, sm_cost_fo = self.loss(cur_trajs, gt_trajs, cost_volume, semantic_pred, lane_divider, drivable_area, target_points)
        else:
            loss, gt_cost_fc, gt_cost_fo, sm_cost_fc, sm_cost_fo = self.loss(cur_trajs, gt_trajs, cost_volume, semantic_pred, lane_divider, drivable_area, target_points)
            loss = 0
        ...
        final_traj, CS = self.select(cur_trajs, cost_volume, semantic_pred, lane_divider, drivable_area, target_points) # (B, n_future, 3)
        ...
        if self.training:
            loss = loss*0.5 + (F.smooth_l1_loss(final[:,:,:2], gt_trajs[:,:,:2], reduction='none')*torch.tensor([10., 1.], device=loss.device)).mean()


        return loss, output_traj, CS, cur_trajs, gt_cost_fc, gt_cost_fo, final_traj, sm_cost_fc, sm_cost_fo
