...
def parse_args():
    ...
    parser.add_argument("--cfg-path", required=True, default="eval_configs/minigpt4_eval.yaml", help="path to configuration file.")
...
def mean_blob_coordinates(image, x, y):
    image = (image[:,:,2]>0)
    labeled_array, num_features = label(image)
    blob_label = labeled_array[y, x]
    blob_coordinates = np.argwhere(labeled_array == blob_label)
    mean_x = np.mean(blob_coordinates[:, 1]).astype(np.int8)
    mean_y = np.mean(blob_coordinates[:, 0]).astype(np.int8)
